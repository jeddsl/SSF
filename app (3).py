  ...
  try {
    ...
    sc.textContent = '(' + (function gpStack(){
      ...
      function updateScales(cards) {
        /* Fade earlier cards as later ones stack on top — NO transform:scale.
           CSS transform on an ancestor creates a new containing block for
           position:fixed descendants, which traps Streamlit's fullscreen
           modal inside the card and makes it impossible to dismiss. */
        cards.forEach(function(card, i){
          var above = 0;
          ...
          var op = Math.max(1 - above * 0.10, 0.65);
          card.style.transform = '';   /* always clear — never set transform */
...
""", height=0, scrolling=False)
    ...
    with _c1:
            ...
            # ── Plot BOTH frontiers (blue was previously missing) ─────────────
            if len(std_blue) > 2:
                ax.plot(std_blue, ret_blue, color=BLUE, lw=2.0, zorder=3,
                        label="MV Frontier (all assets)")
            if len(std_green) > 2:
                ax.plot(std_green, ret_green, color=GREEN, lw=2.0, zorder=4,
                        label=f"MV Frontier (ESG \u2265 {esg_thresh:.1f})")

            # ── Collect all plotted points to compute axis limits ─────────────
            ...
            all_rets = (list(ret_blue) + list(ret_green)
                        ...
                        + [r*100 for r in mu])
            all_stds = [v for v in all_stds if v == v]   # drop NaN
            all_rets = [v for v in all_rets if v == v]

            x_min_d  = min(all_stds);  x_max_d = max(all_stds)
            y_min_d  = min(all_rets);  y_max_d = max(all_rets)
            x_range  = max(x_max_d - x_min_d, 1.0)
            y_range  = max(y_max_d - y_min_d, 1.0)
            x_pad    = x_range * 0.14
            y_pad    = y_range * 0.14

            # CMLs — extend from 0 to right axis edge so they cross the chart
            _cml_end = x_max_d + x_pad
            if sp_tan_esg > 1e-9 and len(std_green) > 0:
                sd_cml2 = np.linspace(0, _cml_end, 300)
            ...
            if sp_tan_all > 1e-9 and len(std_blue) > 0:
                sd_cml = np.linspace(0, _cml_end, 300)
            ...
            # ── Zoom axes to actual data — don't start from zero ─────────────
            ax.set_xlim(max(0, x_min_d - x_pad * 1.5), x_max_d + x_pad)
            ax.set_ylim(y_min_d - y_pad, y_max_d + y_pad)
    ...
    with _c2:
        # ── Build the full ESG–SR frontier (inverted-U shape) ─────────────────
        #
        # The frontier in (ESG, SR) space is the upper envelope of all
        # achievable (portfolio-ESG, portfolio-SR) pairs — analogous to the
        # MV frontier in (σ, E[R]) space.
        #
        # It has two sides, both anchored at the unconstrained tangency:
        #
        #   LEFT  side  (ESG < tangency ESG):
        #     Constrain w·esg ≤ τ and maximise SR.  The portfolio is forced
        #     away from high-ESG assets → SR falls as τ decreases.
        #
        #   RIGHT side  (ESG > tangency ESG):
        #     Constrain w·esg ≥ τ and maximise SR.  The portfolio is forced
        #     toward high-ESG assets → SR falls as τ increases.
        #
        # Together they form the inverted-U with its peak at the unconstrained
        # tangency (the "no-ESG-constraint" portfolio).
        # ──────────────────────────────────────────────────────────────────────
        _uncw0 = np.ones(n) / n
        _unc_res = minimize(
        ...
        _esg_lo   = float(np.dot(_unc_w, esg_scores))   # ESG of unconstrained tangency
        _esg_min  = float(np.min(esg_scores))            # minimum achievable ESG
        _esg_hi   = float(np.max(esg_scores)) * 0.999   # hard upper limit

        _sr_curve = []; _esg_x_curve = []

        # ── LEFT side: w·esg ≤ τ,  τ from _esg_min up to just below _esg_lo ──
        _left_targets = np.linspace(_esg_min * 1.001, _esg_lo * 0.995, 40)
        for _tau in _left_targets:
            try:
                _r = minimize(
                ...
                if _r.success and port_sd(_r.x, cov) > 1e-9:
                    _actual_esg = float(np.dot(_r.x, esg_scores))
                    # Only keep points where the constraint is genuinely binding
                    # (i.e. the portfolio ESG is truly below _esg_lo)
                    if _actual_esg < _esg_lo - 0.01:
                        _esg_x_curve.append(_actual_esg)
                        _sr_curve.append(port_sr(_r.x, mu, cov, rf))
            except Exception:
                continue

        # ── RIGHT side: w·esg ≥ τ,  τ from _esg_lo up to _esg_hi ────────────
        _right_targets = np.linspace(_esg_lo, _esg_hi, 60)
        for _tau in _right_targets:
            try:
                _r = minimize(
                    lambda w: -port_sr(w, mu, cov, rf),
                    _unc_w,
